...
const keepDrawingsToggle = document.getElementById('keepDrawingsOnResize');

resolutionPicker.onchange = () => {
    const [newW, newH] = resolutionPicker.value.split('x').map(Number);
    applyResolution(newW, newH, keepDrawingsToggle.checked);
};

function applyResolution(newW, newH, scaleDrawings) {
    timeline.saveFrame(canvas);
    const oldW = canvas.width;
    const oldH = canvas.height;

    const snaps = timeline.frames.map(frame => {
    ...
    timeline.frames = snaps.map(snap => {
        if (!snap) return null;
        const tmp = document.createElement('canvas');
        tmp.width = newW;
        tmp.height = newH;
        ...
        if (scaleDrawings) {
            c.drawImage(snap, 0, 0, newW, newH);
        } else {
            c.drawImage(snap, 0, 0);
        }
        return c.getImageData(0, 0, newW, newH);
    });

    offsetX = (stage.clientWidth - newW * scale) / 2;
    offsetY = (stage.clientHeight - newH * scale) / 2;
    updateView();
...
window.AnimaAPI.onMenuAction('save', () => {
    if (currentFilePath) {
        window.AnimaAPI.saveProjectTo(currentFilePath, packProject());
    } else {
        window.AnimaAPI.saveProject(packProject());
    }
});
...
window.AnimaAPI.onMenuAction('open', (projectData) => {
    ...
    const h = projectData.height || 1080;
    if (w !== canvas.width || h !== canvas.height) {
        resolutionPicker.value = `${w}x${h}`;
        canvas.width = w;
        canvas.height = h;
        onionCanvas.width = w;
...
window.AnimaAPI.onMenuAction('new', () => {
    timeline.frames = new Array(timeline.maxFrames).fill(null);
